...
const styles = {
  ...
  card: { backgroundColor: '#ffffff', borderRadius: '12px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', padding: '40px 36px', textAlign: 'center' },
  logoText: { fontSize: '24px', fontWeight: '700', color: '#3b82f6', marginBottom: '24px', display: 'block' },
...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    ...
    verifyEmail(token)
      .then((data) => { setStatus('success'); setMessage(data.message || 'Your email has been verified.'); })
      .catch((err) => {
        setStatus('error');
        ...
        {status === 'success' && (
          <>
            ...
            <p style={{ ...styles.message, marginTop: '16px' }}>
              You can now <a href="/login">log in</a> to LearnSphere.
            </p>
          </>
        )}
        {status === 'error' && (
