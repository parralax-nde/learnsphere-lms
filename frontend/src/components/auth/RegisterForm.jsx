...
const styles = {
  ...
  strengthBar: { height: '4px', borderRadius: '2px', marginTop: '6px', backgroundColor: '#e5e7eb', overflow: 'hidden' },
  strengthFill: (color, score) => ({
    height: '100%', width: `${(score / 5) * 100}%`, backgroundColor: color, transition: 'width 0.2s, background-color 0.2s',
  }),
  strengthLabel: (color) => ({ fontSize: '12px', color, marginTop: '2px' }),
  checkList: { listStyle: 'none', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px', marginTop: '6px' },
  checkItem: (passed) => ({ fontSize: '12px', color: passed ? '#16a34a' : '#6b7280', display: 'flex', alignItems: 'center', gap: '4px' }),
  submitBtn: {
...
export default function RegisterForm({ onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (err.response?.data?.errors) {
        const mapped = {};
        err.response.data.errors.forEach(({ field, message }) => { mapped[field] = message; });
  ...
  return (
    <form onSubmit={handleSubmit} noValidate style={styles.form}>
      {serverError && <div role="alert" style={styles.errorBox}>{serverError}</div>}
      <div style={styles.fieldGroup}>
        ...
        />
        {fieldErrors.email && <span role="alert" style={styles.fieldError}>{fieldErrors.email}</span>}
      </div>
      <div style={styles.fieldGroup}>
        ...
        {password && (
          <>
            <div style={styles.strengthBar}><div style={styles.strengthFill(color, score)} /></div>
            <span style={styles.strengthLabel(color)}>{label}</span>
            <ul style={styles.checkList}>
              {requirementChecks.map(({ key, label: reqLabel }) => (
                <li key={key} style={styles.checkItem(checks[key])}>{checks[key] ? '✓' : '○'} {reqLabel}</li>
              ))}
            </ul>
          </>
        )}
        {fieldErrors.password && <span role="alert" style={styles.fieldError}>{fieldErrors.password}</span>}
      </div>
